...
@test "yum install file" {
    run_srun --container-remap-root --container-image=centos:8 bash -c 'yum install -y file'
}

@test "yum install emacs-nox" {
    run_srun --container-remap-root --container-image=centos:8 bash -c 'yum install -y emacs-nox'
}
