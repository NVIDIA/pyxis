...
function setup() {
        enroot remove -f pyxis-nginx-test >/dev/null 2>&1 || true
}
...
@test "manual execution of entrypoint" {
    ...
    run_srun --container-image nginx:1.19.1 --container-name pyxis-nginx-test --container-remap-root nginx -v
    run_srun --container-name pyxis-nginx-test cat /etc/nginx/conf.d/default.conf
    grep -q "listen       80;" <<< "${output}"

    run_srun --container-name pyxis-nginx-test --container-remap-root /docker-entrypoint.sh nginx -v
    run_srun --container-name pyxis-nginx-test cat /etc/nginx/conf.d/default.conf
    grep -q "listen  \[::]\:80;" <<< "${output}"
}
